...
class ClientSender(threading.Thread, metaclass=ClientVerifier):
    ...
    def create_message(self):
        ...
        logger.debug(f'Словарь сообщения: {message_dict}')
        ...
        with sock_lock:
            try:
                ...
                logger.info(f'Сообщение для пользователя {to} отправлено')
            except OSError as err:
                if err.errno:
                    logger.critical('Потеряно соединение с сервером.')
                    exit(1)
                else:
                    logger.error('Не удалось передать сообщение. Превышено время ожиданиия.')
    ...
    def run(self):
        ...
        while True:
            ...
            if command == 'message':
                ...
                print('Такой команды не сушествует, попробойте еще раз. Для вывода существующих команд введите help.')
    ...
    def edit_contacts(self):
        ans = input('Для удаления контакта введите del, для добавления add: ')
        if ans == 'del':
