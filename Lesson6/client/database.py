...
class ClientDatabase:
    ...
    def __init__(self, name):
        ...
        history = Table('message_history', self.metadata, Column('id', Integer, primary_key=True),
                        Column('contact', String), Column('direction', String), Column('message', Text),
                        Column('date', DateTime))

        contacts = Table('contacts', self.metadata, Column('id', Integer, primary_key=True),
...
if __name__ == '__main__':
    ...
    test_db.save_message('test2', 'in',
                         f'Привет! Это тестовое сообщение от {datetime.datetime.now()}!')
    test_db.save_message('test2', 'out',
                         f'Привет! Это еще одно тестовое сообщение от {datetime.datetime.now()}!')
